...
abstract class REST_Controller extends CI_Controller {
    ...
    public function validate_request($rule_group)
    {
        if ($this->form_validation->run($rule_group) == FALSE)
        {    // get the first error msg
             foreach($this->form_validation->get_error_array() as $key=>$value){
                $error=$value;break;
             }
             $this->response(['status'=>self::HTTP_BAD_REQUEST, 'msg' => $error], self::HTTP_BAD_REQUEST);
        }
    ...
    {
        $this->response(['status'=>self::HTTP_INTERNAL_SERVER_ERROR, 'msg' => $e->getMessage()], self::HTTP_INTERNAL_SERVER_ERROR);

    }
    
    // Something went wrong
    public function something_went_wrong(){
        $this->response(['status'=>self::HTTP_INTERNAL_SERVER_ERROR, 'msg' => $this->lang->line('something_wrong')], self::HTTP_INTERNAL_SERVER_ERROR);
    }
    ...
    * @return Response
   */
   public function resizeImages($uploadedImage)
   {
      $config_manip = array(
          'image_library' =>'gd2',
      ...
      if (!$this->image_lib->resize()) {
          ...
          $this->response(['status'=>self::HTTP_BAD_REQUEST, 'msg' => strip_tags($this->image_lib->display_errors())], self::HTTP_BAD_REQUEST); 
      }
   
   }
   
    //Function that resizes image.
    function resizeImage($SrcImage,$DestImage='', $MaxWidth=1000,$MaxHeight=700,$Quality=9)
    {   $DestImage=$SrcImage;
        ...
        {   $imageType=image_type_to_mime_type($type);
            ...
            if($imageType=="image/png"){
    	        if(imagepng($NewCanves,$DestImage,9))
    	        {
    	            imagedestroy($NewCanves);
    	            return true;
    	        }
            ...
            if($imageType=="image/gif"){
    	        if(imagegif($NewCanves,$DestImage,9))
    	        {
    	            imagedestroy($NewCanves);
    	            return true;
    	        }
    ...
    function uploadImage($name,$path){
            ...
            if ( ! $this->upload->do_upload($name)) {
                $this->response(['status'=>self::HTTP_BAD_REQUEST, 'msg' => strip_tags($this->upload->display_errors())], self::HTTP_BAD_REQUEST); 
            }else { 
                $uploadedImage = $this->upload->data();
                //$this->resizeImage($uploadedImage['full_path']);
                //$this->compressImage($uploadedImage['full_path']);
    ...
    function compressImage($source_image, $compress_image='') {
        ...
        imagejpeg($source_image, $compress_image, 25);
        } elseif ($image_info['mime'] == 'image/gif') {
        $source_image = imagecreatefromgif($source_image);
        imagegif($source_image, $compress_image, 25);
